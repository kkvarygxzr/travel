...
| **INV-PRICE-02** | Master Harga TUNGGAL: tarif per unit (`vehicles.day_rate`) hanya bisa ditulis lewat `PATCH /api/pricing/unit-rates/{id}` (halaman Pengaturan → Master Harga); `schemas.VehicleCreate/VehicleUpdate` & `routers/vehicles.py` DILARANG membawa/menulis `day_rate`/`price_from`; FE `VehicleFormDialog.jsx` tak boleh punya input harga; `/pricing/quote` WAJIB memakai `resolve_day_rate` agar angka yang dilihat ops = angka yang ditagih | BUG-0133: harga armada diatur di 2 halaman (Pengaturan vs form Armada) → angka saling menimpa & membingungkan; BUG-0132: quote 'Hitung Otomatis' mengabaikan tarif unit → ops menagih beda dari yang dilihat | `scripts/guardrails/verify_price_master.py` (statik) | — |
...
| **INV-REF-02** | `bookings.destination` = RELASI ke master `destinations` (bukan teks bebas): validator satu pintu `services/refs.destination_or_400` dipanggil di jalur create/group/update `routers/bookings.py`; nilai disimpan sebagai NAMA KANONIK master; FE memakai selector dari `GET /bookings/destination-options`; destinasi operasional non-wisata disimpan ber-`status='draft'` agar tak tayang di web publik. RUNTIME: destinasi di luar master → 400 berALASAN destinasi. **Batch 2 (2026-08-29):** `bookings.origin` = relasi master `pickup_points` (`refs.origin_or_400` di create/group/update; quick-add satu pintu `POST /api/pickup-points`; seed + migrasi `scripts/migrate_ssot_batch2.py`); `leads.destination` jalur ERP divalidasi `destination_or_400`; jalur PUBLIK (lead web/landing) memakai `destination_normalize` LUNAK — inbound eksternal tidak ditolak, hanya dikanonikkan bila cocok master. **Batch 3 (2026-08-29):** destinasi PENAWARAN divalidasi master — `routers/quotations.py` create/update via `destination_or_400` (update hanya-bila-diubah); form penawaran WEB PUBLIK (`features/public/Quotation.jsx`) memakai `<select>` berisi opsi master dari endpoint publik baru `GET /api/public/destination-options` (backend publik tetap LUNAK via `destination_normalize` — nilai warisan/di luar master tetap DITERIMA); kelola master satu pintu di halaman `/app/masterdata` (owner+ops_admin) via `GET/PATCH /api/master/pickup-points[/{id}]` & `GET/PATCH /api/master/destinations[/{id}]` — **RENAME = CASCADE** ke bookings/leads/quotations, **NONAKTIF** (`active` publik / `ops_active` operasional, default aktif bila field tak ada) menyembunyikan dari SEMUA selector (filter di `services/refs.py` + endpoint options) dan pemakaian baru DITOLAK; guard `verify_ssot_relations.py` diperluas +5 cek batch 3 → total **17 cek** (quotations, public options, master cascade/nonaktif). **Batch 4 (2026-08-29):** normalisasi LUNAK menutup SEMUA jalur tulis publik/inbound — `services/refs.origin_normalize` (baru, mirror `destination_normalize` vs master `pickup_points`); pemesanan online `services/booking_public.create_booking` menormalkan `origin`+`destination` (booking publik menulis ke koleksi yang SAMA dgn ERP); lead landing (`routers/public.py`) menormalkan `origin`; lead ads (`services/ads.py`) menormalkan `destination`; **preview cascade** di Master Data — `GET /master/destinations` kini melaporkan `used_by_quotations`, dan FE `MasterData.jsx` menampilkan panel konfirmasi berisi jumlah booking/lead/penawaran yang akan ikut berubah SEBELUM rename dieksekusi (`md-confirm-*`, `md-rename-confirm-*`, `md-rename-cancel-*`); guard diperluas +5 statik +1 runtime (probe POST /public/booking 'bandung'/'bali' → dokumen tersimpan WAJIB 'Bandung'/'Bali') → total **23 cek**. **Batch 5 (2026-08-29):** master KOTA baru (koleksi `cities`, `cty_`) — `customers.city` & `partners.city` divalidasi KERAS `refs.city_or_400` (create+update, nama kanonik; kosong=sah), quick-add satu pintu `POST /api/cities`, selector FE `CitySelect` (form pelanggan `cf-city` & mitra `pf-city`), kelola di `GET/PATCH /api/master/cities[/{id}]` (rename CASCADE customers+partners, toggle `active`); `vehicle_type` lead landing dinormalkan LUNAK `refs.vehicle_type_normalize` vs SSOT `pricing.VEHICLE_TYPE_LABELS` (juga `requested_vehicle_type` booking publik); **GABUNG destinasi kembar** `POST /api/master/destinations/{id}/merge` — cascade bookings/leads/quotations ke nama target lalu sumber dinonaktifkan + `merged_into` (tidak ada penghapusan; target wajib aktif & bukan hasil gabungan); **ekspor Excel** `GET /api/master/export` (3 sheet: Titik Jemput/Destinasi/Kota + pemakaian); seed + `scripts/migrate_ssot_batch5.py`; koleksi `cities` terdaftar di verify_schema/validate_compliance/PURGE_COLLECTIONS; guard diperluas +5 statik +2 runtime (kota ngawur → 400; GET /cities) → total **30 cek**. **Batch 6 (2026-08-29):** **UNDO gabungan** — merge kini MENCATAT id dokumen yang ikut pindah (`merged_moved` di dokumen sumber); `POST /api/master/destinations/{id}/unmerge` mengembalikan HANYA dokumen tercatat yang destinasinya masih nama target (yang diubah manual sesudah merge dilewati + dilaporkan `skipped`), lalu sumber aktif kembali (`$unset merged_into/merged_moved`); UI tombol "Batalkan Gabungan" (`md-unmerge-*`) dgn panel konfirmasi ber-jumlah `merged_moved_count`; **kota BENGKEL** ikut master `cities` — `routers/workshops.py` create/update via `city_or_400`, FE `WorkshopFormDialog` pakai `CitySelect` (wsh-city), rename kota CASCADE + pemakaian + sheet Excel kini mencakup workshops; guard +3 statik (unmerge+merged_moved, workshops city_or_400 ≥2, cascade workshops) → total **34 cek** | RC-E/BUG-0135: 'Bromo' vs 'Gunung Bromo' vs 'bromo ' = 3 destinasi berbeda di laporan; paket/penawaran (sudah `destination_id`) tak pernah cocok dgn booking; input bebas = pelanggaran SSOT | `scripts/guardrails/verify_ssot_relations.py` (statik + runtime, bersih-bersih via `purge_guard_bookings`) | migrasi data lama: `scripts/migrate_booking_destinations.py` |
